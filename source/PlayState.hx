...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['You are not made for this', 0],
		['N/A i think', 0.2], //From 0% to 19%
		['HOW THE FU-',  0.4], //From 20% to 39%
		['Practice yourself', 0.5], //From 40% to 49%
		['Meh', 0.6], //From 50% to 59%
		['You got the Spirit', 0.69], //From 60% to 68%
		['Keep trying!', 0.7], //69%
		['You are doing alright', 0.8], //From 70% to 79%
		['Doing Good', 0.9], //From 80% to 89%
		['Doing Great', 1], //From 90% to 99%
		['Not Perfect', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];

	public static var ratingStuffPortuguese:Array<Dynamic> = [
		['N/A... eu acho', 0.2], //From 0% to 19%
		['MAS QUE PO-',  0.4], //From 20% to 39%
		['Pratique mais', 0.5], //From 40% to 49%
		['Meh', 0.6], //From 50% to 59%
		['Voce tem o espirito', 0.69], //From 60% to 68%
		['Continue tentando!', 0.7], //69%
		['Voce esta no caminho certo', 0.8], //From 70% to 79%
		['Indo bem', 0.9], //From 80% to 89%
		['Indo muito bem', 1], //From 90% to 99%
		['Nao Perfeito', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		practiceMode = ClientPrefs.practiceMode;
		...
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		missesTxt.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		accuracyTxt.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		comboTxt.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		ratingTxt.setFormat(Paths.font("vcr.ttf"), 25, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			for (i in 0...opponentStrums.length) {
				...
				setOnLuas('defaultOpponentStrumY' + i, opponentStrums.members[i].y); //know what screw this
				if(ClientPrefs.middleScroll) opponentStrums.members[i].visible = false;
			}
			opponentStrums.forEach((_:StrumNote)->{_.kill();});
			for (note in unspawnNotes) { 
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			...
			accuracyTxt.text = 'Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + ' %';
			comboTxt.text = 'Combo: ' + combo;
			ratingTxt.text = '(' + ratingFC + ') ' + ratingName; 
		}
		if(ClientPrefs.language == 'Portugues') {
			scoreTxt.text = 'Pontos: ' + songScore;//peeps wanted no integer rating
			missesTxt.text = 'Erros: ' + songMisses;
			accuracyTxt.text = 'Precisao: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + ' %';
			comboTxt.text = 'Combo: ' + combo;
			ratingTxt.text = '(' + ratingFC + ') ' + ratingName; 
		}
		if(ratingName == '?') {
...
if (healthBar.percent > 85)
 ...
 iconP2.animation.curAnim.curFrame = 0;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (health > 0.07 && !note.isSustainNote)
			{
				health -= 0.04;
			}
	...
	function StrumPlayAnim(isDad:Bool, id:Int, time:Float) {
		...
		if(spr != null) {
			spr.playAnim('confirm', true);
			spr.resetAnim = 0.25999999999999999999999999999999999999999999999999999;
		}
	}
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			else
			{
				...
				if(ClientPrefs.language == 'Portugues') {
					ratingName = ratingStuffPortuguese[ratingStuffPortuguese.length-1][0];
				}

				// Rating Name
				if(ratingPercent >= 1 && ClientPrefs.language == 'English')
				{
			...
			ratingFC = "";
			if(ClientPrefs.language == 'Portugues') {
			if (sicks > 0) ratingFC = "Perfeito";
			if (goods > 0) ratingFC = "Otimo";
			if (bads > 0 || shits > 0) ratingFC = "Bom";
			if (songMisses > 0 && songMisses < 10) ratingFC = "Certo";
			else if (songMisses >= 10) ratingFC = "...";
			} else {
				if (sicks > 0) ratingFC = "Perfect";
			if (goods > 0) ratingFC = "Great";
			if (bads > 0 || shits > 0) ratingFC = "Good";
			if (songMisses > 0 && songMisses < 10) ratingFC = "Alright";
